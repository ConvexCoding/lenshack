<script lang="ts">
  ...
  let gridColor = 0x0000ff
  let centerGridColor = 0x0000ff
  let shadowScreenColor = 0xffff00
  ...
  if (showSides) {
    for (let i = 0; i <= 10; i++) {
      ...
      line.push(new Vector3(-halfHoriz + horizontalStep * i, verticalOffset, -halfHoriz))
      line.push(
        new Vector3(-halfHoriz + horizontalStep * i, verticalSize + verticalOffset, -halfHoriz)
      )
      lines.push(line)
      line = []
      line.push(new Vector3(halfHoriz, verticalOffset, -halfHoriz + horizontalStep * i))
      line.push(
        new Vector3(halfHoriz, verticalSize + verticalOffset, -halfHoriz + horizontalStep * i)
      )
      lines.push(line)
    }
    for (let i = 0; i <= 5; i++) {
    ...
    dashes = []
    dashes.push(new Vector3(halfHoriz, horizCenterline + verticalOffset, -halfHoriz))
    dashes.push(new Vector3(halfHoriz, horizCenterline + verticalOffset, +halfHoriz))
...
<T.Mesh>
  {#each lines as line}
    <T.Line points={line} material={new LineBasicMaterial({ color: gridColor })} />
  {/each}
...
{#if showShadowScreens}
  ...
  <T.Mesh position={[0, verticalOffset - 0.5, 0]} rotation={[Math.PI / 2, 0, 0]} receiveShadow>
    <T.BoxGeometry args={[horizontalSize, horizontalSize, 0.1]} />
    <T.MeshStandardMaterial side={DoubleSide} color={shadowScreenColor} transparent opacity={0.2} />
  </T.Mesh>
{/if}
